...
#imprimir mejor manera visual las recompensas por votos y temperatura
""" recompensas_xvotos = calcular_recompensas_hora_votos()
for (hora, temp), recompensa in recompensas_xvotos.items():
    print(f"Hora: {hora}, Temp: {temp}, Recompensa: {recompensa}") """


""" esta funcion es para ver los porcentajes de votos por temperatura y hora y poder comparar que la funcion calcular_recompensas_hora_votos este dando porcentajes 
correctos en cuanto a recompensa """
...
#para imprimir de manera bonita los porcentajes de los votos en las diferentes horas con sus temperaturas pero se debe modificar
""" porcentajes = compacion_visual_calculo_porcentajeshxv()
for (hora, temp), porcentaje in porcentajes.items():
    print(f"Hora: {hora}, Temp: {temp}")
    for voto, porc in porcentaje.items():
        print(f"    Voto: {voto}, Porcentaje: {porc:.2f}%") """
